...
class AllowedStudentId extends Model
{
    ...
    protected $fillable = ['student_id', 'education_level', 'used'];

    /**
     * Check if a student id is allowed and not yet used. Optionally validate education level.
     */
    public static function isAllowed(string $studentId, ?string $educationLevel = null): bool
    {
        $query = static::where('student_id', $studentId)->where('used', false);
        if ($educationLevel) {
